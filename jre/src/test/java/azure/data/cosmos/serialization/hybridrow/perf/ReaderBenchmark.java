...
public final class ReaderBenchmark {
    ...
    private static Task RowReaderBenchmarkAsync(Object ctx) {
        ...
        // TODO: C# TO JAVA CONVERTER: There is no equivalent to 'await' in Java:
        Result r = await
        Microsoft.Azure.Cosmos.Serialization.HybridRow.RecordIO.RecordIOStream.ReadRecordIOAsync(context.getInput(),
            record ->
        {
        ...
        {
            Segment _;
            tangible.OutObject<azure.data.cosmos.serialization.hybridrow.recordio.Segment> tempOut__ =
    ...
    private static Result VisitOneRow(Memory<Byte> buffer, LayoutResolver resolver) {
        ...
        return Microsoft.Azure.Cosmos.Serialization.HybridRow.Tests.Perf.RowReaderExtensions.VisitReader(reader.clone());
    }
    ...
    private final static class SingleThreadedTaskScheduler extends TaskScheduler implements Closeable {
        ...
        private void DoWork() {
            while (!this.cancel.IsCancellationRequested) {
                Task item;
                tangible.OutObject<Task> tempOut_item = new tangible.OutObject<Task>();
