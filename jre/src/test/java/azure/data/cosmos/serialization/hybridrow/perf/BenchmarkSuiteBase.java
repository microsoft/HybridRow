...
import azure.data.cosmos.serialization.hybridrow.layouts.LayoutResolverNamespace;
import azure.data.cosmos.serialization.hybridrow.layouts.SystemSchema;
import azure.data.cosmos.serialization.hybridrow.recordio.RecordIOStream;
import azure.data.cosmos.serialization.hybridrow.recordio.Segment;
...
public class BenchmarkSuiteBase {
    ...
    protected static Result LoadOneRow(Memory<Byte> buffer, LayoutResolver resolver,
                                       tangible.OutObject<HashMap<Utf8String, Object>> rowValue) {
        RowBuffer row = new RowBuffer(buffer.Span, HybridRowVersion.V1, resolver);
        tangible.RefObject<RowBuffer> tempRef_row =
            new tangible.RefObject<RowBuffer>(row);
        RowReader reader = new RowReader(tempRef_row);
        row = tempRef_row.argValue;
        tangible.RefObject<RowReader> tempRef_reader =
            new tangible.RefObject<RowReader>(reader);
