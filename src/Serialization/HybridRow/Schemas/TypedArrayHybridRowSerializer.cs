...
namespace Microsoft.Azure.Cosmos.Serialization.HybridRow.Schemas
{
    ...
    public struct TypedArrayHybridRowSerializer<T, TSerializer> : IHybridRowSerializer<List<T>>
        where TSerializer : struct, IHybridRowSerializer<T>
    {
        public IEqualityComparer<List<T>> Comparer => TypedArrayComparer.Default;

        public Result Write(ref RowBuffer row, ref RowCursor scope, bool isRoot, TypeArgumentList typeArgs, List<T> value)
        {
            Result r = LayoutType.TypedArray.WriteScope(ref row, ref scope, typeArgs, out RowCursor childScope);
            if (r != Result.Success)
            {
                return r;
            }
        ...
        {
            Result r = LayoutType.TypedArray.ReadScope(ref row, ref scope, out RowCursor childScope);
            if (r != Result.Success)
            {
                value = default;
                return r;
        ...
        {
            public static new readonly TypedArrayComparer Default = new TypedArrayComparer();
