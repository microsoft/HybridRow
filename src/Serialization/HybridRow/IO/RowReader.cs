...
namespace Microsoft.Azure.Cosmos.Serialization.HybridRow.IO
{
    ...
    public ref struct RowReader
    {
        ...
        public RowReader(ReadOnlyMemory<byte> buffer, HybridRowVersion version, LayoutResolver resolver)
        {
            ...
            this.row = new RowBuffer(MemoryMarshal.AsMemory(buffer).Span, version, resolver);
        ...
        /// <param name="version">The version of the Hybrid Row format to used to encoding the buffer.</param>
        /// <param name="resolver">The resolver for UDTs.</param>
        /// <param name="checkpoint">Saved point from a previous iteration.</param>
        public RowReader(ReadOnlyMemory<byte> buffer, HybridRowVersion version, LayoutResolver resolver, in Checkpoint checkpoint)
        {
            // TODO: remove this cost-cast when ReadOnlyRowBuffer exists.
            // The cost-cast implied by MemoryMarshal.AsMemory is only safe here because:
            // 1. Only READ operations are performed on the row.
        ...
        /// <summary>Read the current field as a nested, structured, sparse scope.</summary>
        public Result ReadScope<T, TSerializer>(out T value)
            where TSerializer : struct, IHybridRowSerializer<T>
        {
            return default(TSerializer).Read(ref this.row, ref this.cursor, isRoot: false, out value);
        }
