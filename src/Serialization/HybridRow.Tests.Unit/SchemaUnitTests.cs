...
namespace Microsoft.Azure.Cosmos.Serialization.HybridRow.Tests.Unit
{
    ...
    {
        private const string SchemaFile = @"TestData\CoverageSchema.hrschema";
        ...
        public void ParseNamespace()
        {
            // Test empty schemas.
            {
                string[] emptyNamespaceJsonInput =
                {
                    ...
                    @"{'version': 'v2', 'name': null, 'schemas': null }",
                };
                ...
                foreach (string json in emptyNamespaceJsonInput)
                {
                    ...
                    Assert.AreEqual(SchemaLanguageVersion.V2, n1.Version);
                }
            }
            ...
            // Test simple schemas and schema options.
            {
                ...
                // Version defaults properly when NOT specified.
                Assert.AreEqual(SchemaLanguageVersion.V2, n1.Version);
            }

            {
                ...
                Assert.AreEqual(SchemaLanguageVersion.V1, n1.Schemas[0].Version, "Json: {0}", json);
            ...
            // Test basic schema with primitive columns.
            {
                ...
                Assert.AreEqual(SchemaLanguageVersion.Unspecified, n1.Schemas[0].Version, "Json: {0}", json);
                Assert.AreEqual(new SchemaId(1), n1.Schemas[0].SchemaId, "Json: {0}", json);
                Assert.AreEqual(TypeKind.Schema, n1.Schemas[0].Type, "Json: {0}", json);
        ...
        [TestMethod]
        [Owner("jthunter")]
        [DeploymentItem(SchemaUnitTests.SchemaFile, "TestData")]
        public void ParseNamespaceExample()
        {
            Namespace n1 = SchemaUtil.LoadFromHrSchema(SchemaUnitTests.SchemaFile);
            string json2 = Namespace.ToJson(n1);
            Namespace n2 = Namespace.Parse(json2);
            string json3 = Namespace.ToJson(n2);
        ...
        public void ParseSchemaPrimitives()
        {
            ...
            foreach (dynamic expected in expectedSchemas)
            {
                ...
                string nsSchema = $"{{'schemas': [{tableSchema}]}}";
                Schema s = Namespace.Parse(nsSchema).Schemas[0];
        ...
        public void ParseSchemaArray()
        {
            ...
            foreach (dynamic expected in expectedSchemas)
            {
                ...
                string udtSchema = "{'name': 'myUDT', 'id': 1, 'type': 'schema' }";
                string nsSchema = $"{{'schemas': [{tableSchema}, {udtSchema}]}}";
                Schema s = Namespace.Parse(nsSchema).Schemas[0];
                Assert.AreEqual(1, s.Properties.Count, "Json: {0}", expected.Json);
                Property p = s.Properties[0];
        ...
        public void ParseSchemaObject()
        {
            ...
            dynamic[] expectedSchemas =
            {
                new { Json = @"{'path': 'b', 'type': {'type': 'int8' }}", Type = TypeKind.Int8 },
        ...
        public void ParseSchemaPartitionPrimaryKeys()
        {
            ...
            foreach (var expected in expectedSchemas)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < s.PrimaryKeys.Count; i++)
                    {
                        Assert.AreEqual(expected.CK[i].Path, s.PrimaryKeys[i].Path, "PK: {0}, CK: {1}", expected.JsonPK, expected.JsonCK);
                        Assert.AreEqual(expected.CK[i].Dir, s.PrimaryKeys[i].Direction, "PK: {0}, CK: {1}", expected.JsonPK, expected.JsonCK);
                    }
                }
                catch (Exception ex)
                {
