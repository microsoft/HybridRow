...
namespace Microsoft.Azure.Cosmos.Serialization.HybridRow.Tests.Unit
{
    ...
    {
        private readonly Hotels hotelExample = new Hotels
        {
        ...
        {
            this.hotelLayout = CustomerSchemaHrSchema.LayoutResolver.Resolve((SchemaId)HotelsHybridRowSerializer.SchemaId);
            this.guestLayout = CustomerSchemaHrSchema.LayoutResolver.Resolve((SchemaId)GuestsHybridRowSerializer.SchemaId);
        }
        ...
        public void CreateHotel()
        {
            ...
            row.InitLayout(HybridRowVersion.V1, this.hotelLayout, CustomerSchemaHrSchema.LayoutResolver);

            Hotels h1 = this.hotelExample;
            RowCursor root = RowCursor.Create(ref row);
            this.WriteHotel(ref row, ref root, h1);

            root = RowCursor.Create(ref row);
            Hotels h2 = this.ReadHotel(ref row, ref root);

            Assert.IsTrue(default(HotelsHybridRowSerializer).Comparer.Equals(h1, h2));
        }
        ...
        public void FrozenHotel()
        {
            ...
            ResultAssert.InsufficientPermissions(
                this.PartialUpdateHotelAddress(
                    ref row,
                    ref root,
                    new Address { Street = "300B Brownie Way" }));
        }
        ...
        public void CreateGuest()
        {
            ...
            row.InitLayout(HybridRowVersion.V1, this.guestLayout, CustomerSchemaHrSchema.LayoutResolver);
            ...
            Guests g1 = new Guests
            {
                ...
                Emails = new List<string> { "president@whitehouse.gov" },
                Addresses = new Dictionary<string, Address>(default(Utf8HybridRowSerializer).Comparer)
                {
                    ["home"] = new Address
                    {
            ...
            Guests g2 = this.ReadGuest(ref row, ref rc2);
            Assert.IsTrue(default(GuestsHybridRowSerializer).Comparer.Equals(g1, g2));
        ...
        private static Address ReadAddress(ref RowBuffer row, ref RowCursor addressScope)
        {
            ...
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref addressScope, c, out string value));
            a.Street = value;
            Assert.IsTrue(addressLayout.TryFind("city", out c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref addressScope, c, out value));
            a.City = value;
            Assert.IsTrue(addressLayout.TryFind("state", out c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadFixed(ref row, ref addressScope, c, out value));
            a.State = value;
        ...
        private static PostalCode ReadPostalCode(ref RowBuffer row, ref RowCursor postalCodeScope)
        {
            ...
            ResultAssert.IsSuccess(c.TypeAs<LayoutInt32>().ReadFixed(ref row, ref postalCodeScope, c, out int value));
            pc.Zip = value;
        ...
        private static void WriteAddress(ref RowBuffer row, ref RowCursor addressScope, TypeArgumentList typeArgs, Address a)
        {
            Layout addressLayout = CustomerSchemaHrSchema.LayoutResolver.Resolve(typeArgs.SchemaId);
            Assert.IsTrue(addressLayout.TryFind("street", out LayoutColumn c));
            ...
            CustomerExampleUnitTests.WritePostalCode(ref row, ref postalCodeScope, c.TypeArgs, a.PostalCode);
            addressScope.Skip(ref row, ref postalCodeScope);
        }

        private static void WritePostalCode(ref RowBuffer row, ref RowCursor postalCodeScope, TypeArgumentList typeArgs, PostalCode pc)
        {
            Layout postalCodeLayout = CustomerSchemaHrSchema.LayoutResolver.Resolve(typeArgs.SchemaId);
            ...
            if (pc.Plus4 != default)
            {
                Assert.IsTrue(postalCodeLayout.TryFind("plus4", out c));
                postalCodeScope.Find(ref row, c.Path);
                ResultAssert.IsSuccess(c.TypeAs<LayoutInt16>().WriteSparse(ref row, ref postalCodeScope, pc.Plus4));
            }
        }

        private void WriteHotel(ref RowBuffer row, ref RowCursor root, Hotels h)
        {
            ...
            CustomerExampleUnitTests.WriteAddress(ref row, ref addressScope, c.TypeArgs, h.Address);
            root.Skip(ref row, ref addressScope);
        }

        private void WriteGuest(ref RowBuffer row, ref RowCursor root, Guests g)
        {
            ...
            if (g.Addresses != null)
            {
                ...
                foreach (KeyValuePair<string, Address> pair in g.Addresses)
                {
                    ...
                    CustomerExampleUnitTests.WriteAddress(ref row, ref addressScope, t1.TypeArgs, pair.Value);
        ...
        private Hotels ReadHotel(ref RowBuffer row, ref RowCursor root)
        {
            Hotels h = new Hotels();
            Assert.IsTrue(this.hotelLayout.TryFind("hotel_id", out LayoutColumn c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref root, c, out string value));
            h.Id = value;
            Assert.IsTrue(this.hotelLayout.TryFind("name", out c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref root, c, out value));
            h.Name = value;
            Assert.IsTrue(this.hotelLayout.TryFind("phone", out c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref root, c, out value));
            h.Phone = value;
        ...
        private Guests ReadGuest(ref RowBuffer row, ref RowCursor root)
        {
            Guests g = new Guests();
            Assert.IsTrue(this.guestLayout.TryFind("guest_id", out LayoutColumn c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutGuid>().ReadFixed(ref row, ref root, c, out Guid value));
            g.Id = value;
            Assert.IsTrue(this.guestLayout.TryFind("first_name", out c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref root, c, out string value2));
            g.FirstName = value2;
            Assert.IsTrue(this.guestLayout.TryFind("last_name", out c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref root, c, out value2));
            g.LastName = value2;
            Assert.IsTrue(this.guestLayout.TryFind("title", out c));
            ResultAssert.IsSuccess(c.TypeAs<LayoutUtf8>().ReadVariable(ref row, ref root, c, out value2));
            g.Title = value2;
            ...
            if (c.TypeAs<LayoutTypedMap>().ReadScope(ref row, ref addressesScope, out addressesScope) == Result.Success)
            {
                ...
                TypeArgument t0 = tupleType.TypeArgs[0];
                TypeArgument t1 = tupleType.TypeArgs[1];
                g.Addresses = new Dictionary<string, Address>(default(Utf8HybridRowSerializer).Comparer);
                ...
                while (addressesScope.MoveNext(ref row, ref pairScope))
                {
                    ...
                    Address value3 = CustomerExampleUnitTests.ReadAddress(ref row, ref addressScope);
                    g.Addresses.Add(key, value3);
