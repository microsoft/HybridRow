...
namespace cdb_hr
{
  ...
  struct TypedArrayHybridRowSerializer final
  {
    struct TypedArrayComparer;
    using value_type = std::vector<typename TSerializer::owning_type>;
    using owning_type = value_type;
    using const_reference = const value_type&;
    using comparer_type = TypedArrayComparer;
                 ...
                 const_reference value) noexcept
    {
      ...
      auto [r, childScope] = LayoutLiteral::TypedArray.WriteScope(row, scope, typeArgs);
      if (r != Result::Success)
      {
        return r;
      }
    ...
    {
      cdb_core::Contract::Assert(!isRoot);
      auto [r, childScope] = LayoutLiteral::TypedArray.ReadScope(row, scope);
      if (r != Result::Success)
      {
        return {r, value_type{}};
      }
    ...
    struct TypedArrayComparer final
    {
