...
namespace Microsoft.Azure.Cosmos.Serialization.HybridRow.Tests.Perf
{
    ...
    {
        private Namespace schemas;

        [TestInitialize]
        public void TestInitialize()
        {
            this.schemas = TestData.LoadFromHrSchema(TestData.SchemaFile);
            this.DefaultResolver = new LayoutResolverNamespace(this.schemas, SystemSchema.LayoutResolver);
        }
        ...
        private async Task GenerateBenchmarkAsync(string schemaName, int outerLoopIterations, string expectedFile)
        {
            ...
            if (!allMatch)
            {
                await BenchmarkSuiteBase.WriteAllRowsAsync(expectedFile, this.schemas, resolver, resolver.Resolve(tableSchema.SchemaId), rows);
